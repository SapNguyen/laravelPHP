...
class PaymentController extends Controller
{
    ...
    public function insert(Request $request)
    {
        if (session('login') != 'true') {
            ...
            if (!session('payment')) {
                ...
                $last_order = DB::table('order')->latest('order_id')->first();
                if ($last_order) {
                    $r_id = $last_order->order_id + 1;
                } else {
                    $r_id = 1;
                }
                $total = 0;
                ...
                for ($i = 0; $i < count($arr); $i++) {
                    ...
                    DB::table('product_order')->updateOrInsert([
                        'order_id' => $r_id,
                        'size' => $size,
                        'color' => $color,
                        'product_id' => $pid,
                ...
                return redirect('/account/order/0');
            }
        }
    }
    ...
    public function loadPayment_api(Request $request)
    {
        try {
            // if (session('login') != 'true') {
            //     return redirect('/login');
